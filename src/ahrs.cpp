...
Quat madgwickCorrectionStep(const Quat& q_pred,
                            const Vec3& a,
                            const Vec3& m,
                            const Vec3& gyro,
                            double beta,
...
bool readIMU(Vec3& gyro, Vec3& accel, Vec3& mag) {
    sensors_event_t gyro_event, accel_event, mag_event, temp_event;

    // Read LSM6DSOX (gyro + accel)
    if (!sox.getEvent(&accel_event, &gyro_event, &temp_event)) {
        return false;
    }
